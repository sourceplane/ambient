...
- **`cloudflare_hyperdrive_config`**: Hyperdrive gateway resource
  - **Name**: `{namespacePrefix}ambient-{environment}` (e.g., `ambient-stage`)
...
| `orgName` | string | `ambient` | Organization name (for tags and secret paths) |
| `owner` | string | `ambient` | Repository owner (for tags) |
| `repo` | string | `ambient` | Repository name (for tags and secret paths) |
| `namespace` | string | `ambient` | Namespace (for labels and naming conventions) |
...
1. **`supabase` component** (same repo)
   ...
   - This component reads credentials from the secret path: `sourceplane/ambient/supabase/{environment}`
...
- **Secret path**: `sourceplane/ambient/supabase/{environment}`
...
aws secretsmanager get-secret-value --secret-id sourceplane/ambient/supabase/stage
```

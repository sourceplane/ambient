# lists-worker — runbook

## How it deploys

Merges to `main` converge automatically. The `lists` schema is created by the
`db-migrate` component (migration `280`).
...
- **503 on every route**: `PLATFORM_DB` unbound or the `lists` schema missing —

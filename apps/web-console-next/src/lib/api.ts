...
// deleted; all wire I/O now goes through the typed `ambient` resource
...
//   - `createClient` — constructs a `ambient` against a target, wired
...
//     `Promise<T>` + thrown `ambientError` into the same shape the
...
import { ambient, ambientError, type ClientOptions } from "@saas/sdk";
import { apiEdgeWorkersDevUrl } from "./app-config";

export type ApiClient = ambient;
...
export function createClient(target: ApiTarget, token: string | null): ambient {
  const opts: ClientOptions = { baseUrl: target.url };
  if (token) opts.auth = { kind: "bearer", token };
  return new ambient(opts);
}

/**
 * Adapt an SDK promise into the `ApiResult<T>` envelope. Typed
 * `ambientError` subclasses surface as `{ ok: false }` carrying the

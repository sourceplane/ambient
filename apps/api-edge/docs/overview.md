...
Part of the ambient runtime: a Cloudflare Worker deployed per
environment (`stage`, `prod`; `dev` is verify-only). Public entry point: `https://ambient-api-edge-{stage,prod}.rahulvarghesepullely.workers.dev`
...
- **web-console-next** — Next.js 15 + opennextjs/cloudflare delivery of the ambient web console (per-environment, Workers + Static Assets)

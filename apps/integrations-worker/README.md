...
Your backend holds one ambient API key and **zero GitHub credentials**.
...
const ambient = new ambient({
  ...
  token: process.env.AMBIENT_API_KEY!, // service-principal key
});

// Mint a token scoped to exactly the repos + permissions you need.
// TTL ≤ 1h; mint per task and let it expire — never store it.
const { token } = await ambient.integrations.issueGithubToken(ORG_ID, {

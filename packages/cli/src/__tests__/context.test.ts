...
describe("resolveConfigDir", () => {
  it("respects AMBIENT_CONFIG_DIR override", () => {
    const prev = process.env["AMBIENT_CONFIG_DIR"];
    process.env["AMBIENT_CONFIG_DIR"] = "/tmp/cli-override";
    try {
      expect(resolveConfigDir()).toBe("/tmp/cli-override");
    } finally {
      if (prev === undefined) delete process.env["AMBIENT_CONFIG_DIR"];
      else process.env["AMBIENT_CONFIG_DIR"] = prev;
    }
  });
});

...
`ambient` — first-class TypeScript CLI for the ambient control
...
This package is internal to the `ambient` monorepo and is not
...
```
ambient login    [--api-url=URL] [--token=BEARER]
ambient logout
ambient whoami
```
...
```
ambient org list
ambient org use <org-id>
ambient org members
ambient project list
```

Writes (Task 0101):

```
ambient org invite <email> [--role=ROLE] [--idempotency-key=KEY] [--org=ORG_ID]
ambient project create <name> [--idempotency-key=KEY]
ambient env create <project-id> <name> [--idempotency-key=KEY]
ambient api-key create <name> [--scope=SCOPE] [--idempotency-key=KEY]
ambient webhook create <url> [--event=EVENT[,EVENT2,...]] [--idempotency-key=KEY]
```

Cross-resource reads (Task 0101):

```
ambient usage summary    [--metric=METRIC] [--from=ISO] [--to=ISO]
ambient billing summary
ambient audit list       [--limit=N] [--cursor=CURSOR] [--category=CAT] [--all]
```
...
context (`ambient org use <org-id>`). Only `org invite` accepts an
...
The shipped V1 is **token-paste**: `ambient login` prompts for a
...
- `FileTokenStore` fallback: `~/.config/ambient/credentials.json`,
  mode **0600**, parent directory mode **0700**.

Active organization context lives at
`~/.config/ambient/config.json` (mode 0644, not a secret). Override
both via `AMBIENT_CONFIG_DIR` (used by tests).

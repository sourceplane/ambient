...
ambient TypeScript SDK — typed, dependency-free, runtime-agnostic client
for the ambient control plane API.

## Install (workspace)

This package is internal to the `ambient` monorepo. Add it as a
...
```ts
import { ambient } from "@saas/sdk";

const client = new ambient({
  baseUrl: "https://api.ambient.sh",
  auth: { kind: "bearer", token: process.env["AMBIENT_TOKEN"]! },
});
...
All non-2xx responses throw a typed subclass of `ambientError`. Branch on
...
| `ambientError` (base)   | _any_                 | _any_          |

Unknown error codes (forward-compatible — e.g. a future `quota_exceeded`)
decode to the base `ambientError` carrying the raw envelope. Non-JSON 5xx
...
A custom `fetch` implementation can be injected via `new ambient({ fetch })`.

// Base HTTP transport for the ambient SDK.
//
// Goals:
...
// - Typed errors: every non-2xx response is decoded into a `ambientError`
...
export interface ClientOptions {
  /**
   * Base URL of the ambient api-edge worker, e.g.
   * `https://api.ambient.sh`. Trailing slash is stripped.
   */
  baseUrl: string;
 ...
 * `ambient` (the client class wired up in `index.ts`).
 */
export class Transport {
  ...
  constructor(options: ClientOptions) {
    ...
    if (typeof fetchImpl !== "function") {
      throw new TypeError(
        "ambient SDK requires a fetch implementation. Pass `fetch` in options on platforms without a global.",
      );
    }
...
export function generateRequestId(): string {
  ...
  throw new Error(
    "ambient SDK requires the Web Crypto API (globalThis.crypto). No source of randomness was found.",
  );
}

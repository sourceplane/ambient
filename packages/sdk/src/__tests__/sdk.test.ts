...
describe("Transport — success path", () => {
  it("unwraps the data envelope on GET", async () => {
    ...
    const client = new ambient({ baseUrl: "https://api.test", fetch });
...
describe("Error decoding — typed branches", () => {
  ...
  const cases: Array<[string, number, new (...args: never[]) => ambientError]> = [
  ...
  it("forward-compat: unknown error code → base ambientError", async () => {
    ...
    try {
      ...
      expect(err).toBeInstanceOf(ambientError);
      expect(err).not.toBeInstanceOf(BadRequestError);
      expect((err as ambientError).code).toBe("future_quota_exceeded");
    }
  });
